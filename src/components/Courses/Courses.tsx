...
function Courses() {
  ...
  const handleSearchClick = useCallback(
    (searchKey: any) => setSearchString(searchKey),
    [setSearchString]
  );
  const coursesWithAuthorNames = useSelector(getCoursesWithAuthorNames);
  const noCourses = coursesWithAuthorNames.length === 0;
  const filteredCourses = searchString
    ? searchCourses(searchString, coursesWithAuthorNames)
  ...
  const renderCourses = () =>
    noCourses ? <EmptyCourseList /> : courseListElement;
