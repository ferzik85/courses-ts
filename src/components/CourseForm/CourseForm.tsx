...
function CourseForm() {
	...
	const handleAddAuthorToCourse = (e: React.MouseEvent, authorId: string) => {
		...
		const authorWithTheSameIdAlreadyExists =
			courseAuthorIds.find((courseAuthorId) => courseAuthorId === authorId) !=
			null;
	...
	const handleRemoveAuthorFromCourse = (
		e: React.MouseEvent | null,
		authorId: string
	) => {
		e?.preventDefault();
		setCourseAuthorIds([
			...courseAuthorIds.filter((courseAuthorId) => courseAuthorId !== authorId)
		]);
	};
	...
	return (
		<div className={styles.courseForm}>
			<h3 className={styles.courseFormHeader}>
			...
			<div className={styles.courseFormBody}>
				...
				>
					...
					<LabeledInput
						...
						inputClassName={styles.courseFormTitle}
					/>
					<LabeledInput
						name="Description"
						value={description}
					...
					<div className={styles.createAuthorsPanel}>
						<div className={styles.addAuthorsPanel}>
							...
							{authors.map((author: any) => (
								<AuthorItem
									...
									id={author.id}
									name={author.name}
									onAddClick={handleAddAuthorToCourse}
						...
						<div className={styles.courseAuthorsPanel}>
							...
							{courseAuthorListIsEmpty ? (
								<p className={styles.courseAuthorsPanelEmptyList}>
